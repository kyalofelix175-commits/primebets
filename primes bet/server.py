...
# ----------------------------------------------------
ODDS_API_KEY = os.getenv("ODDS_API_KEY", "7b3f031a4f75b509e5681704af5e0a84")
ODDS_API_URL = "https://api.the-odds-api.com/v4/sports/soccer_pakistan_super_league/odds/"

MPESA_CONSUMER_KEY = os.getenv("MPESA_CONSUMER_KEY", "HPtek3s8gtYvkgmYY3UOfIV09kKSC2e6pGmSbqla0giOdK2d")
MPESA_CONSUMER_SECRET = os.getenv("MPESA_CONSUMER_SECRET", "VCSucyK5rDBn5kvJNjP98xNcvUAnOf57pewboSwZlL2Cfx9QIachwgDzZjIGcYWd")
...
def auth_signup():
    ...
    if phone in users_db:
        return jsonify({"success": False, "message": "Account already registered under this phone"}), 409
    ...
    return jsonify({"success": True, "message": "Registration successful! Proceed to Sign In."})
...
def auth_signin():
    ...
    if not user or not check_password_hash(user['password_hash'], password):
        return jsonify({"success": False, "message": "Invalid combination details entered"}), 401

    session_token = str(uuid.uuid4())
    users_db[phone]['session_token'] = session_token

    response = make_response(jsonify({"success": True, "message": "Access authorized!", "balance": user['balance']}))
...
def get_fixtures():
    """Fetches up-to-date matches dynamically from Odds API or falls back safely to mock arrays."""
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            processed = []
            for item in raw_fixtures[:10]: # Return top 10 matches
                commence_time = datetime.fromisoformat(item['commence_time'].replace('Z', '+00:00'))
                is_live = datetime.now(timezone.utc) >= commence_time
                
                home_odd, away_odd = 2.10, 2.80 
                if item.get('bookmakers'):
                    markets = item['bookmakers'][0].get('markets', [])
                    if markets and markets[0].get('outcomes'):
                        outcomes = markets[0]['outcomes']
                        home_odd = next((o['price'] for o in outcomes if o['name'] == item['home_team']), 2.0)
                        away_odd = next((o['price'] for o in outcomes if o['name'] == item['away_team']), 2.0)
...
def place_betslip():
    ...
    if not phone: return jsonify({"success": False, "message": "Authentication required"}), 401
    ...
    if users_db[phone]['balance'] < stake: return jsonify({"success": False, "message": "Insufficient floating wallet balance"}), 400
    
    # Rule 5: Odds Cap Constraint Verification
    if total_odds > 1000.0:
        return jsonify({"success": False, "message": "Maximum betslip accumulator limits restricted to 1000 odds!"}), 400
...
def cancel_betslip():
    ...
    for game in slip['selections']:
        ...
        if datetime.now(timezone.utc) >= commence_time:
            return jsonify({"success": False, "message": "Cancellation locked! One or more games have started or expired."}), 400
...
def mpesa_deposit():
    ...
    if amount < 5: 
        return jsonify({"success": False, "message": "Minimum allowable system transaction value is KSH 5"}), 400

    token = get_mpesa_token()
    if not token: return jsonify({"success": False, "message": "Upstream communication authentication link down"}), 500
...
def mpesa_withdraw():
    ...
    if not check_password_hash(users_db[phone]['password_hash'], password):
        return jsonify({"success": False, "message": "Security verification failure: Invalid credentials"}), 403

    # Rule 5: Baseline bounds limit validation
    if amount < 5: 
        return jsonify({"success": False, "message": "Minimum transaction payout bounds set to KSH 5"}), 400
    ...
    if users_db[phone]['balance'] < total_deduction:
        return jsonify({"success": False, "message": f"Insufficient balances. Required amount with 5% transaction processing fee: KSH {total_deduction:.2f}"}), 400

    token = get_mpesa_token()
    if not token: return jsonify({"success": False, "message": "Upstream communication core runtime error"}), 500
    ...
    try:
        ...
        return jsonify({"success": False, "message": "Downstream execution channel error. Balance returned safely."}), 500
