...
def get_fixtures():
    """Fetches real-time upcoming and live matches containing BOTH odds and live scores."""
    # To get live scores and real-time odds, we target the live/upcoming odds endpoint
    ODDS_AND_SCORES_URL = "https://api.the-odds-api.com/v4/sports/soccer_epl/odds/" # Example for EPL, or keep your dynamic URL
    params = {
        'apiKey': ODDS_API_KEY,
        'regions': 'eu',
        'markets': 'h2h',
    ...
    try:
        ...
        # Make sure your URL points to a valid sport or use upcoming general endpoint
        res = requests.get("https://api.the-odds-api.com/v4/sports/upcoming/odds/", params=params, timeout=5)
        ...
        if res.status_code == 200:
            ...
            for item in raw_fixtures:
                ...
                # --- FIX: Safe Parsing of Real Decimal Odds ---
                home_odd = 2.00
                away_odd = 2.00
                
                bookmakers = item.get('bookmakers', [])
                if bookmakers:
                    # Target the first available bookmaker (e.g., 1xBet, Bet365)
                    markets = bookmakers[0].get('markets', [])
                    if markets:
                        outcomes = markets[0].get('outcomes', [])
                        for outcome in outcomes:
                            if outcome['name'] == item['home_team']:
                                home_odd = outcome.get('price', 2.00)
                            elif outcome['name'] == item['away_team']:
                                away_odd = outcome.get('price', 2.00)

                # --- FIX: Safe Parsing of Real Live Scores ---
                home_score = 0
                away_score = 0
                if scores:
                    for score_item in scores:
                        if score_item['name'] == item['home_team']:
                            home_score = int(score_item.get('score', 0))
                        elif score_item['name'] == item['away_team']:
                            away_score = int(score_item.get('score', 0))
            ...
            print(f"Odds API Error Status: {res.status_code} - {res.text}")
            
    except Exception as e:
        print(f"Exception Caught during fixture parsing: {e}")

    # Fallback to mock data only if API fails or quota limit is reached
    mock_fixtures = [
        {
            "id": "mock_1", "sport_title": "English Premier League", "sport_key": "soccer_epl",
            "home_team": "Manchester City", "away_team": "Arsenal", "is_live": True, "completed": False,
            "odds": {"home": 1.85, "away": 3.40}, "scores": {"home": 2, "away": 1}, "time_played": "74'" 
        },
        {
            "id": "mock_2", "sport_title": "La Liga", "sport_key": "soccer_spain_la_liga",
            "home_team": "Real Madrid", "away_team": "Barcelona", "is_live": False, "completed": False,
            "odds": {"home": 2.15, "away": 2.95}, "scores": {"home": 0, "away": 0}, "time_played": None
        }    
    ]
    return jsonify({"success": True, "fixtures": mock_fixtures})
