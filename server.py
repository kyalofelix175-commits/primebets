...
def get_fixtures():
    ...
    # 💡 CRITICAL: Use the /odds/ endpoint. It includes live scores automatically if a game is active!
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for item in raw_fixtures:
                ...
                # The Odds API embeds a 'scores' array inside the /odds/ endpoint for live games!
                scores = item.get('scores', None)
                is_live = scores is not None and not is_completed
                
                # 1. Parse live scores safely
                home_score = 0
                away_score = 0
                if scores:
                ...
                # 2. Parse betting odds safely
                home_odd, away_odd = 2.10, 2.80 # System baseline fallbacks
                if item.get('bookmakers'):
                    markets = item['bookmakers'][0].get('markets', [])
                    if markets and markets[0].get('outcomes'):
                        outcomes = markets[0]['outcomes']
                        home_odd = next((o['price'] for o in outcomes if o['name'] == item['home_team']), 2.10)
                        away_odd = next((o['price'] for o in outcomes if o['name'] == item['away_team']), 2.80)
    ...
    # Sandbox fallback matches (Keep structure matching the processed output above)
    mock_fixtures = [
        {
            "id": "mock_1", 
...
def serve_root_images(filename):
    # Safely serves image files sitting directly in your root directory
