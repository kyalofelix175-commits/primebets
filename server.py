...
def get_fixtures():
    ...
    try:
        ...
        res = requests.get(ODDS_AND_SCORES_URL, params=params, timeout=5)
...
def filter_by_category(fixtures, category):
    ...
    if category == 'live':
        return [f for f in fixtures if f['is_live']]
        
    if category == 'epl':
        return [f for f in fixtures if 'premier league' in f['sport_title'].lower()]
        
    if category == 'soccer':
        # Catches anything labeled Soccer or Football
        return [f for f in fixtures if 'soccer' in f['sport_title'].lower() or 'football' in f['sport_title'].lower()]
        
    if category == 'volleyball':
        return [f for f in fixtures if 'volleyball' in f['sport_title'].lower()]
        
    if category == 'basket':
        return [f for f in fixtures if 'basketball' in f['sport_title'].lower()]
        
    # 'others' filter fallback
    known_sports = ['premier league', 'soccer', 'football', 'volleyball', 'basketball']
    return [f for f in fixtures if not any(x in f['sport_title'].lower() for x in known_sports) and not f['is_live']]
