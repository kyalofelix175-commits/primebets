...
def get_fixtures():
    """Fetches real-time upcoming and live matches, with backend category filtering."""
    # 1. Get the category requested by the frontend (defaults to 'all')
    category = request.args.get('category', 'all').lower()
    
    ODDS_AND_SCORES_URL = "https://api.the-odds-api.com/v4/sports/upcoming/odds/"
    params = {
        'apiKey': ODDS_API_KEY,
        'regions': 'eu',
        'markets': 'h2h',
    ...
    try:
        ...
        res = requests.get(ODDS_AND_SCORES_URL, params=params, timeout=10)
        raw_fixtures = res.json() if res.status_code == 200 else []
        ...
        for item in raw_fixtures:
            ...
            # --- [Your existing score and odds parsing logic goes here] ---
            home_score, away_score = 0, 0 # (Keep your loops here...)
            home_odd, away_odd = 2.10, 2.80 # (Keep your outcomes here...)
            ...
            fixture_data = {
                ...
                "time_played": "Live" if is_live else None
            }
            processed.append(fixture_data)

        # 2. FILTER THE LIVE DATA BASE ON CATEGORY
        filtered_fixtures = filter_by_category(processed, category)
        return jsonify({"success": True, "fixtures": filtered_fixtures})
            
    except Exception as e:
        print(f"Exception Caught: {e}")

    # Sandbox fallback matches
    mock_fixtures = [
        {"id": "mock_1", "sport_title": "English Premier League", "home_team": "Manchester City", "away_team": "Arsenal", "is_live": True, "completed": False, "odds": {"home": 1.85, "away": 3.40}, "scores": {"home": 2, "away": 1}, "time_played": "74'"},
        {"id": "mock_2", "sport_title": "La Liga", "home_team": "Real Madrid", "away_team": "Barcelona", "is_live": False, "completed": False, "odds": {"home": 2.15, "away": 2.95}, "scores": {"home": 0, "away": 0}, "time_played": None}    
    ]

    # 3. FILTER THE MOCK DATA BASE ON CATEGORY IF BACKEND FAILS
    filtered_mock = filter_by_category(mock_fixtures, category)
    return jsonify({"success": True, "fixtures": filtered_mock, "note": "Displaying system sandbox matches."})


# 💡 HELPER FUNCTION FOR FILTERING

def filter_by_category(fixtures, category):
    if category == 'all' or not category:
        return fixtures
        
    if category == 'live':
        return [f for f in fixtures if f.get('is_live')]
        
    # ⚽ Soccer / Football matches
    if category == 'soccer':
        return [
            f for f in fixtures 
            if 'soccer' in f.get('sport_title', '').lower() 
            or 'football' in f.get('sport_title', '').lower()
            or 'epl' in f.get('sport_title', '').lower() # Keeps EPL under the main Soccer tab too
        ]
        
    # 🏆 EPL specific matches
    if category == 'epl':
        return [
            f for f in fixtures 
            if 'epl' in f.get('sport_title', '').lower() 
            or 'premier league' in f.get('sport_title', '').lower()
        ]
        
    # 🏐 Volleyball matches
    if category == 'volleyball':
        return [f for f in fixtures if 'volleyball' in f.get('sport_title', '').lower()]
        
    # 🏀 Basketball matches
    if category == 'basket':
        return [
            f for f in fixtures 
            if 'basket' in f.get('sport_title', '').lower() 
            or 'nba' in f.get('sport_title', '').lower()
        ]
        
    # 🌀 'others' filter fallback
    known_keywords = ['epl', 'premier league', 'soccer', 'football', 'volleyball', 'basket', 'nba']
    return [
        f for f in fixtures 
        if not any(x in f.get('sport_title', '').lower() for x in known_keywords) 
        and not f.get('is_live')
    ]
